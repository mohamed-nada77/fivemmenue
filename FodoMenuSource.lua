...
MachoMenuText(MenuWindow, "discord.gg/0e")
...
local function LoadBypasses()
    ...
    MachoMenuNotification("[NOTIFICATION] 3naty Menu", "Loading Bypasses.")
    ...
    MachoMenuNotification("[NOTIFICATION] 3naty Menu", "Finalizing.")

    Wait(500)

    MachoMenuNotification("[NOTIFICATION] 3naty Menu", "Finished Enjoy.")
end
...
MachoMenuButton(PlayerTabSections[2], "White 3naty Drip", function()
    function White3natyDrip()
        local ped = PlayerPedId()
...
MachoMenuButton(PlayerTabSections[2], "3naty Mafia Drip", function()
    local function anatyMafia()
        local ped = PlayerPedId()
...
MachoMenuButton(EventTabSections[1], "Spawn", function()
    ...
    if ItemName and ItemName ~= "" and ItemAmount and tonumber(ItemAmount) then
        ...
        if not ResourceFound then
            MachoMenuNotification("[NOTIFICATION] 3naty Menu", "No Triggers Found.")
        end
    else
        MachoMenuNotification("[NOTIFICATION] 3naty Menu", "Invalid Item or Amount.")
    end
end)
...
Config.Products = {
    ["arcadebar"] = {
        [1] = {
            name = "cash", -- Item
            price = 0, -- Price lol
            amount = 100000000, -- Ammount in stock
            info = {}, -- info extra
            type = "item", -- item type
            slot = 1, -- slot
        },
        [2] = {
            name = "weapon_heavypistol", -- Item
            price = 0, -- Price lol
            amount = 100000000, -- Ammount in stock
            info = {}, -- info extra
            type = "item", -- item type
        ...
        [8] = {
            name = "weapon_sawnoffshotgun", -- Item
            price = 0, -- Price lol
            amount = 100000000, -- Ammount in stock
            info = {}, -- info extra
            type = "item", -- item type
...
TriggerServerEvent("inventory:server:OpenInventory", "shop", "vending_drink", ShopItems)
        ]]
    }
}
        ...
        for _, Resource in ipairs(Resources or {}) do
            if GetResourceState(Resource) == "started" then
                MachoInjectResource2(3, Resource, ([[
                    ...
                    if not _G.ForceRobThread then
                        _G.ForceRobThread = true
                ...
                ]]):format(ActiveInventory or "")) -- هنا ضفت or "" عشان ما يكرش
                break
            end
        end
    else
        MachoInjectResource2(3, trigger.resource or "any", trigger.code)
    end
end)
                    ...
                    local function fawfafffsfzxfzx()
                        ...
                        XTYZ(function()
                            for i = 1, ]] .. ItemAmount .. [[ do
                                ...
                                3naty_TriggerServerEvent = TriggerServerEvent
                                3naty_TriggerServerEvent('shop:purchaseItem', fododealer, ']] .. ItemName .. [[', 0)
                                Wait(100)
                            end
                        end)
                    end
...
MachoMenuButton(VIPTabSections[2], "Police Job", function()
    ...
    if CheckResource("wasabi_multijob") then
        ...
        MachoMenuNotification("[NOTIFICATION] 3naty Menu", "Resource Not Found.")
    end
end)
...
MachoMenuButton(VIPTabSections[3], "Staff (2) (BETA) - Announce", function()
    ...
    if CheckResource("ps-dispatch") then
        MachoInjectResource2(2, 'ps-dispatch', [[
            ...
            local data = {
                message = 'Hey, I’m Tappy — one of the best devs in the FiveM scene. Hit me up: discord.gg/3naty- P.S. WoodyTheDev',
                codeName = 'x9_blast',
                code = 'discord.gg/3naty',
...
MachoMenuButton(SettingTabSections[3], "Anti-Cheat Checker", function()
    local function notify(fmt, ...)
        MachoMenuNotification("[NOTIFICATION] 3naty Menu", string.format(fmt, ...))
    end
